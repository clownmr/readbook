...
{
    /// <summary>
    ///这一典型样品控制器implements登录/注销/佣金工作流为本地和外部账户。
    ///的encapsulates登录服务的数据与用户互动之大。这是在大数据的只读存储器和不能被用来生产！
    ///服务的互动提供了一个方法的用户界面和沟通identityserver为验证和上下文检索
    /// </summary>
    public class AccountController : Controller
    {
        ...
        /// <summary>
        /// Handle postback from username/password login  处理回发的用户名/密码登录
        /// </summary>
        [HttpPost]
        ...
        public async Task<IActionResult> Login(LoginInputModel model)
        {
            ...
            {
                // validate username/password against in-memory store 在内存存储中验证用户名/密码
        ...
        public async Task<IActionResult> Logout(LogoutViewModel model)
        {
            ...
            //得到的上下文信息（客户姓名，后注销重定向URI和iframe的联合signout）
        ...
        public async Task<IActionResult> ExternalLoginCallback(string returnUrl)
        {
            ...
            // retrieve claims of the external user 检索外部用户的索赔
            var claims = tempUser.Claims.ToList();

            //试图确定身份的独特的外部用户最普通类型的索赔，索赔是子和nameidentifier
            //取决于外部提供商的一些其他类型的索赔可能会被用来
            ...
            //从集合中移除用户ID索赔索赔和移动用户标识属性
            // remove the user id claim from the claims collection and move to the userId property
            // also set the name of the external authentication provider 还设置外部身份验证提供程序的名称.
